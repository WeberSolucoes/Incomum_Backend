...
def list_all() -> Response:
    lojas = Cidade.objects.all().order_by('cid_descricao')
...
def search_cidades(request):
    ...
    if len(query) >= 3:
        with connection.cursor() as cursor:
            sql_query = """
                SELECT cid_codigo, cid_descricao, cid_estado
                FROM cidade
            ...
            # Inclui o UF (cid_estado) no retorno
            cidade_list = [{'label': cidade[1], 'value': cidade[0], 'uf': cidade[2]} for cidade in cidades]
